...
adb push denials /sdcard/___DENS_
...
if [ ! -z "$(cat denials)" ]; then
        adb shell su -c logcat -c > /dev/null
        adb shell su -c supolicy --save /system/SAVED_POLICIES
        echo "Policies have been saved on your phone at /system/SAVED_POLICIES"
	echo
